...
async def chat(request: Request) -> Any:
    """Endpoint del Laboratorio del CRM: procesa UN turno y devuelve la
    respuesta del agente SIN enviarla por WhatsApp.

    Cuerpo: {conversationId, text, waIdentity}
      - conversationId: conversación is_test del CRM (se resuelve el contexto
        por id, multi-tenant correcto).
      - text: mensaje del cliente simulado (una línea de la persona).
      - waIdentity: identidad sintética del contacto de prueba.

    Devuelve {replies: [...], handoff: reason|null} — el CRM persiste las
    respuestas y construye el transcript para el reporte.
    """
    ctx: AppContext = request.app.state.ctx
    api_key = request.headers.get("X-API-Key", "")
    if ctx.settings.crm_bot_api_key and api_key != ctx.settings.crm_bot_api_key:
...
# ------------------------------------------------------------- analytics ---
# Fase 1: dashboard de medicamentos más buscados. Endpoint protegido por
# X-API-Key (misma que el webhook). Filtros: providerId (obligatorio),
# desde/hasta (ISO), top N (default 20).

@router.get("/analytics/top-meds")
async def analytics_top_meds(
    request: Request,
    provider_id: str = "",
    desde: str = "",
    hasta: str = "",
...
# ---------------------------------------------------- pacientes crónicos ---
# Fase 2: lista de pacientes con condición crónica detectada (clasificación
# automática por consultas repetidas de medicamentos crónicos). El CONSENT
# se gestiona en la Fase 3; aquí solo se expone el estado.

@router.get("/analytics/chronic-patients")
async def analytics_chronic_patients(
    request: Request,
    provider_id: str = "",
    consentidos: str = "",
) -> JSONResponse:
